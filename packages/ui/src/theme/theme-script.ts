...
export function themeScript({ storageKey = STORAGE_KEY, appearanceKey = APPEARANCE_KEY }: ThemeScriptOptions = {}): string {
  ...
  return (
    ...
    // appearance (.dark): preference from localStorage then cookie; `system`/absent → matchMedia.
    ...
    "if(ap==='dark'){d.classList.add('dark');}else if(ap==='light'){d.classList.remove('dark');}else{d.classList.toggle('dark',!!(window.matchMedia&&window.matchMedia('(prefers-color-scheme: dark)').matches));}" +
    "}catch(e){}})();"
  );
}
