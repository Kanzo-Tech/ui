...
export const TourPositioner = (
  ...
  <ArkTour.Positioner
    className={cn(
      ...
      // `p-4` only on the dialog-type step, matching DialogPositioner. The content is
      // `w-full max-w-md`, so without it a viewport narrower than 448px puts the card flush
      // against both edges. Tooltip-type steps are anchored to a target and must not inherit
      // the inset. (Upstream Shark omits this on Tour; Dialog here has always had it.)
  ...
  return (
    <ArkTour.Title
      className={cn(
        ...
        // Reserve inline-end room so the absolutely-positioned close X never overlaps the
        // title — the same guard Dialog and Popover already carry. Upstream Shark omits it on
        // Tour, which is why a long title runs straight under the button there.
