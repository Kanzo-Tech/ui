 ...
 * in real time.
 *
 * Layout: Customize on its own. Both fumadocs theme controls are gone — the one in its own row
 * (`themeSwitch={{ enabled: false }}` on the layout) and the `ThemeSwitch` that used to sit here.
 * The latter calls `useTheme()`, so with next-themes disabled it became a button that changes
 * nothing, and appearance lives inside the panel now. The Customize trigger is our own
 * `DialogTrigger` (the same Ark Dialog `PreferencesRoot` provides), styled in
