...
Two parts are not in the tree. `ColorPickerView` is the column that holds the area and the sliders,
...
`InputGroup` it sits in, and it is `pointer-events-none`: a preview, never a target.
