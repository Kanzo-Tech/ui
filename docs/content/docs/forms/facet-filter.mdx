...
disappears and every row moves up. The caller knows whether the column is `provider` or
`subject_uri`; the component only knows how many rows survived the last filter, which is not the same
question.
</Callout>
...
`Combobox` makes — so `coruna` finds `A Coruña`. It matches the row's **label**, falling back to its
