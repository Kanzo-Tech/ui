...
| `CardRadioGroup` | `value: string` | `onValueChange` | **a bare `string`** |
...
| `DateField` | `value: string \| null` | **`onChange`** | **a bare `string \| null`** |
...
`RadioGroup`, `Slider` and the date controls (`DatePicker`, and `DateField` which wraps it) **do
not**. For those, pass `invalid` to the control as well as to the `Field`; it is the one place the
...
Ours, not Ark's — the `options` sugar over `RadioGroupCard`. Because it is ours, the callback is a
plain string, so `onValueChange={field.handleChange}` is the entire wiring. It takes `invalid`
directly, and `FieldLegend` inside a `FieldSet` gives the group its heading.

<ComponentPreview componentName="form/tanstack" fileName="example-card-radio-group" hasMaxHeight={false} showBorders={false} />
...
`DateField` is ours, and its whole reason to exist is this: a plain ISO string (`YYYY-MM-DD`, or
`YYYY-MM-DDTHH:mm` with `withTime`) on the outside, `onChange` rather than `onValueChange`, and no
date objects anywhere in your form state.

<ComponentPreview componentName="form/tanstack" fileName="example-date-field" hasMaxHeight={false} showBorders={false} />

<Callout type="warn">
**Use `DateField`, not the `DatePicker` compound, for a form value.** `DatePicker` is Ark's
machine, and its `value` is a `DateValue[]` — objects you can only build with `parseDate` from
`@internationalized/date`, which the library does not re-export. Driving it from a form means
adding that package to your app and converting in both directions on every change. Reach for the
compound when you need to rearrange its parts, and lift the value with `DateField` when you need
it in a form.
</Callout>
...
Grouped `FieldSet`s, three kinds of choice control, an array of maintainers with two sub-fields
per row, a form-level `Alert` for a server error, and a submit button that reports its own state
through `form.Subscribe`.
...
const schema = z.object({
  contacts: z
    .array(z.object({ id: z.string(), address: z.email("Enter a valid email address.") }))
