...
<ComponentPreview componentName="use-chart" />
...
```tsx
import { useChart } from "@kanzo-tech/ui/analytics";
```
...
It must be rendered **inside** a [`ChartRoot`](/docs/data-display/charts) — it throws otherwise.
Use [`useChartOptional`](/docs/hooks/use-chart-optional) for a part that also has to work
standalone.
...
Takes no arguments. Throws `useChart must be used within a <ChartRoot>.` outside one.
...
| `coordinator` | `Coordinator` | for a part that queries alongside the plot — usually via [`useChartQuery`](/docs/hooks/use-chart-query) |
