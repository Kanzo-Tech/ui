...
the chart lives in, so a scoped palette override on an ancestor wins.

<Callout type="warn">
Resolving is also what makes it a **snapshot**. Nothing re-calls `color` when the theme flips or the
tenant's palette is swapped. If your sink is only a `style`, pass `var(--chart-N)` straight through
