...
```tsx
import { useChartContextOptional } from "@kanzo-tech/ui/analytics";
```
...
[`useChartContext`](/docs/hooks/use-chart-context) throws outside a `ChartRoot`, and that is the right default: a
...
```ts
function useChartContextOptional(): ChartContextValue | null;
```

Takes no arguments. Returns `null` outside a `ChartRoot`; otherwise the same object
[`useChartContext`](/docs/hooks/use-chart-context#returns) returns.
...
[two kinds of children](/docs/analytics/charts#two-kinds-of-children).
