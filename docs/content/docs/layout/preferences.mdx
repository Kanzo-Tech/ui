...
<Callout type="info" title="Colour is a preference — a hue is not">
  ...
  *part* of a palette a hue at a time; a theme is written once and chosen whole. **Authoring one is a
  ...
  studio](/docs/showcases/theme-studio), which is where the "copy the CSS" half lives, aimed at
...
**The semantics are GitHub's now, and that is new.** They pair a day theme with a night theme
because their themes are single-mode; ours used to each carry both, so a card meant *which document
this user wears on the light side*. A theme is one mode, so a card is the light theme.
...
The grid is the other half. Stacked at full width, the themes this site publishes took over
