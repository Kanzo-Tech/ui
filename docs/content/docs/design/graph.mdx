...
**Two things it must never become.** A source that takes column names — they come from the manifest or
they do not come. That used to be stated against a sibling, `duckBoundedSource`, which took two ordinary
relations and the names that made sense of them; the sibling is deleted and this package sends one
source, so the rule no longer has a bad example to point at and has to stand on its own. And a **fetch on
the camera path**: the moment a camera move costs a request before it can compute a URL, `viewport` is
back.
...
*Held by* `packages/graph/src/duck-source.ts`, `openCorpus`; `packages/graph/src/index.test.ts`, "keeps
the neighbourhood question deleted, and says what it was for" — which is where `ExploringSource` went,
and why the seam this rule names is still the one `expand` arrives through.
...
the aggregate far view deleted, names and all". Two tests used to stand here as well — one running the
stride over arrays, one checking both reads compute the same divisor — and both belonged to sources that
are deleted. What runs the arithmetic now is the browser, in `docs/showcases/graph-bench`.
...
*Held by* `packages/graph/src/duck-source.ts`, `anchorCte` and `longEnough` — **and by nothing else, which
is a loss this line states rather than hides.** Both halves ran over arrays in
`packages/graph/src/graph-model.test.ts` until `memorySource` was deleted, and the SQL they mirror has no
stub that can reach it: `openCorpus` boots wasm and fetches manifests before it builds a query. The
figures above and the archive on screen are what is left.
...
*Held by* `packages/graph/src/slice-client.ts`, `SliceRead`, and by the measurements above. The three
tests that used to stand here — the predicate reaching both reads, two queries where there were three,
and the canvas' exemption from its own clause — drove `duckBoundedSource`, which is the source that was
deleted; `packages/graph/src/duck-source.test.ts` says so at the top rather than quietly shrinking.
