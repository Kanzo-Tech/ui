...
`options` out and it offers every distinct value it finds; pass them to fix the order, relabel a
raw value or give it an icon.
...
`facetFilterFn` compares by equality against the selected values, which is what the menu means.
...
the menu stays open across selections — a filter is a set, not a single choice.
