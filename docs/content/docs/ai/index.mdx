...
roles is `assistant`. That is domain knowledge, and `DESIGN.md`'s first admission rule keeps the
generic vocabulary free of it, the same way it keeps graphs out by name.
...
purchase across two installs. `decisions/the-ai-surfaces-are-their-own-package.md` records the cost
as well as the call.
</Callout>
...
`decisions/ai-elements-is-a-source-not-a-reference.md`:
