...
There used to be a ten-export `ContextMenu*` family here, nine members of which were `data-slot`
renames of the parts above — `ContextMenuItem` was `<MenuItem data-slot="context-menu-item">` and
nothing more. Only the trigger was real, and it now lives beside `MenuTrigger` under Ark's own
name for it. The names are gone with no aliases; a context menu is a `Menu` whose trigger is a
`MenuContextTrigger`.
