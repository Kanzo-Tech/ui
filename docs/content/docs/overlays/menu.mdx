...
<Callout type="info">
**Menu vs [Context Menu](/docs/overlays/context-menu):** they are the *same menu*. `ContextMenu`
renders `Menu` and swaps one part — the trigger. Use `Menu` when a visible control opens it
(a button, an avatar, a "⋯"); use `ContextMenu` when the trigger is a *region* you right-click.
