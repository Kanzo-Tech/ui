...
import { SAMPLE_MAPPING } from "./data.js";
...
/**
 * The two-sided IDE at full viewport: our Sidebar as the explorer, WorkspaceLayout as
 * canvas + resizable dock, a real CodeMirror editor on the canvas, and the status bar
 * driving the panels. This is the arrangement WorkspaceLayout documents — and the only
 * way to judge ⌘\, the dock drag and the collapse-to-icons behaviour.
 */
export function WorkspaceScene() {
  const [source, setSource] = useState(SAMPLE_MAPPING);

  return (
    // `h-svh`: SidebarProvider defaults to `min-h-svh` (right for page-scrolling apps),
    // but a shell that owns the viewport needs a RESOLVED height — WorkspaceLayout sizes
    // itself with `height: 100%`, and a percentage against an auto-height parent falls
    // back to auto, collapsing the splitter and the editor to content height.
    <SidebarProvider className="h-svh">
      ...
      {/* `min-w-0` so the inset can shrink below its content's intrinsic width (a flex
          child defaults to `min-width:auto`), `min-h-0` so it does not grow past the
          viewport, and `overflow-hidden` so the shell scrolls internally. */}
      <SidebarInset className="min-h-0 min-w-0 overflow-hidden">
        ...
        >
          <div className="flex h-full flex-col">
            <Toolbar
              actions={
                <>
                  ...
                  <Button
                    onClick={() =>
                      toast.create({ title: "Run started", type: "info" })
                    }
                    size="xs"
                  >
                    <PlayIcon />
              ...
              // Back lives here, in the chrome — legible, in the tab order, and not
              // sitting on top of the code it would otherwise cover.
              leading={
              ...
              left={
                <Breadcrumbs
                  // Shrunk to the toolbar's own scale — the primitive keeps Shark's
                  // `text-sm` default, and the thin IDE strip opts down to 11px.
            ...
            />
            {/* No `overflow-auto` here: CodeMirror's own `.cm-scroller` scrolls. Wrapping
                it in a second scroller collapses the editor to its content height and
                leaves the canvas half empty. */}
